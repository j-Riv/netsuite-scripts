...
define(['N/runtime', 'N/search', 'N/email', 'N/file'],
  function (runtime, search, email, file) {
    ...
    function execute() {
      ...
      const dir = runtime.getCurrentScript().getParameter('custscript_apparel_re_order_dir');
    ...
    const loadSearch = () => {
      ...
      const itemSearch = search.load({
        id: runtime.getCurrentScript().getParameter('custscript_apparel_re_order_search')
      });
      ...
      pagedData.pageRanges.forEach(function (pageRange) {
        ...
        page.data.forEach(function (result) {
          itemResults.push({
            ...
            'locationquantityavailable': result.getValue({ name: 'formulanumeric' }),
            'custitem_sp_apparel_size_re_order': result.getValue({ name: 'custitem_sp_apparel_size_re_order' })
          });
        })
      });
    ...
    const sendEmail = (items, attachmentId) => {
      ...
      const emailRecipient = runtime.getCurrentScript().getParameter('custscript_apparel_re_order_email_id');
      const emailList = runtime.getCurrentScript().getParameter('custscript_apparel_re_order_email_list').split(',');
      ...
      if (items.length > 0) {
        ...
        html = '<p>The following have dropped below the re-order point at Main Warehouse as of (' + new Date() + ')</p>' +
        ...
        html = '<p>No items have dropped below the re-order point at Main Warehouse as of (' + new Date() + ')</p>' +
